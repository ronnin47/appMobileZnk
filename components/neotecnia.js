import { View, Text, StyleSheet, TextInput, TouchableOpacity, Image, ScrollView, Alert, FlatList,ImageBackground, KeyboardAvoidingView, Platform } from 'react-native';
...
export const Neotecnia = () => {
  const [objetosNeo, setObjetosNeo] = useState([]);
  ...
  const consumirObjetosNeo = async () => {
    try {
        //console.log('URL consumida:', `${API_BASE_URL}/consumirObjetosNeo`);
      const response = await axios.get(`${API_BASE_URL}/consumirObjetosNeo`);
      ...
      if (Array.isArray(objetosNeo)) {
        ...
        setObjetosNeo(ordenadosPorNivel);
      }
    } catch (error) {
      console.error('Error al consumir dispositivos Neotecnicos:', error.message);
    }
  };
  ...
  const crearObjetoVacioEnDB = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/insertObjetoNeo`, objetoVacio, {
  ...
  const guardarCambiosObjeto = async () => {
    try {
      await axios.put(`${API_BASE_URL}/updateObjetoNeo/${nuevoObjeto.idobjeto}`, nuevoObjeto, {
        headers: { 'Content-Type': 'application/json' },
      });
      ...
      console.error('Error al actualizar el dispositivo Neotecnico:', error.message);
    }
  };

  const eliminarObjeto = async (idobjeto) => {
    try {
      await axios.delete(`${API_BASE_URL}/deleteObjetoNeo/${idobjeto}`);
      setObjetosNeo((prev) => prev.filter((obj) => obj.idobjeto !== idobjeto));
      setNuevoObjeto(null);
      alert('Objeto eliminado correctamente');
    } catch (error) {
      console.error('Error al eliminar dispositivo Neotecnico:', error.message);
      alert('No se pudo eliminar el dispositivo Neotecnico.');
    }
  };
  ...
  const objetosFiltrados = objetosNeo
    .filter((obj) => {
  ...
  const grupos = [
    {
      ...
      color: 'white',
      data: comunes,
      textoNoHay: 'No hay dispositivos comunes.',
    },
    {
      key: 'pocoComunes',
      ...
      color: '#4CAF50',
      data: pocoComunes,
      textoNoHay: 'No hay dispositivos poco comunes.',
    },
    {
      key: 'raros',
      ...
      color: '#2196F3',
      data: raros,
      textoNoHay: 'No hay dispositivos raros.',
    },
    {
      key: 'unicos',
      ...
      color: '#FF9800',
      data: unicos,
      textoNoHay: 'No hay dispositivos únicos.',
    },
  ];
...
const fondoUrl = "https://res.cloudinary.com/dzul1hatw/image/upload/v1763048799/c24705ca9662f07250a63d634ea87b79_euv1o3.jpg"; 
  return (

    <ImageBackground
          source={{ uri: fondoUrl  }}
          style={{ flex: 1, opacity:1, backgroundColor:"rgba(3, 3, 3, 0.07)" }}
          resizeMode='cover'
        >
      ...
      {estatus === 'narrador' && (
        <>
          <View style={{ alignItems: 'center', marginTop: 10 }}>
            ...
            >
              <Text style={styles.botonTexto}>+ disositivo</Text>
            </TouchableOpacity>
          </View>
          ...
          {nuevoObjeto && (
            <View style={styles.cargarObjeto}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 30, gap: 40 }}>
                ...
                >
                  <Text style={styles.botonTexto}>Eliminar dispositivo</Text>
                </TouchableOpacity>
